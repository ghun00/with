...
**Stack**: React 18 + TypeScript + Vite, Tailwind CSS v4, React Router (routes are declared inline in `src/App.tsx` — no separate router file), TanStack Query, Supabase (Auth · Postgres · RLS · Storage). Login is **Kakao OAuth only** (Supabase Auth Kakao provider) — this is why the invite flow (below) hinges on OAuth `redirectTo` round-trips. Deployed as a static SPA on Vercel; `vercel.json` rewrites every path to `/index.html` (client-side routing), so there is no server — all backend logic is Supabase (RLS + the one Edge Function).
